	...
	for _, finding in pairs(findings_for_root(root)) do
		...
		if bufnr and vim.api.nvim_buf_is_loaded(bufnr) then
			...
			table.insert(diagnostics_by_buffer[bufnr], {
				lnum = finding.start_line - 1,
				end_lnum = finding.end_line,
				col = 0,
				end_col = 0,
...
function M.at_cursor(root)
	...
	if ids_by_line and ids_by_line[line] then
		return findings_for_root(root)[ids_by_line[line]]
	end
	local path = project.relative(root, vim.api.nvim_buf_get_name(bufnr))
	for _, finding in pairs(findings_for_root(root)) do
		if finding.path == path and line >= finding.start_line and line <= finding.end_line then
			return finding
		end
	end
end
