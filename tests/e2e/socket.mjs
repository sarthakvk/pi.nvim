...
const extension = new URL("../../pi-extension/index.ts", import.meta.url).pathname;
const child = spawn("script", ["-qfec", `pi --extension ${JSON.stringify(extension)}`, "/dev/null"], {
...
try {
  await waitFor(() => output.includes("gpt-5.6"), "interactive Pi did not start");
  child.stdin.write("/nvim-bridge enable\r");
  const stateHome = process.env.XDG_STATE_HOME || join(process.env.HOME, ".local", "state");
  const runtime = join(process.env.XDG_RUNTIME_DIR || join(stateHome, "nvim", "run"), "pi.nvim");
  let descriptor;
  await waitFor(() => {
    descriptor = readdirSync(runtime)
      .filter((name) => name.endsWith(".json"))
      .map((name) => ({ name, value: JSON.parse(readFileSync(join(runtime, name), "utf8")) }))
  ...
  socket.write(JSON.stringify({ id: "hello", type: "hello", version: 1, root: project }) + "\n");
  await waitFor(() => received.includes('"id":"hello"'), "bridge handshake did not respond");
  const response = JSON.parse(received.split("\n").find((line) => line.includes('"id":"hello"')));
