...
function expectedTextMatches(root: string, finding: Finding): boolean {
  try {
    ...
    return splitLines(text).slice(finding.start_line - 1, finding.end_line).join("\n") === finding.expected_text;
  } catch { return false; }
}
  ...
  socket.on("data", (chunk: Buffer) => {
    ...
    while (true) {
      ...
      if (message.type === "hello") {
        ...
        if (id) runtime.findings.delete(id); else runtime.findings.clear();
        runtime.ctx.sessionManager.appendCustomEntry(FINDINGS_CLEAR_TYPE, { id });
        broadcastFindings(runtime); respond(socket, message.id, { cleared: id });
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerCommand("nvim-bridge", {
    ...
    handler: async (args, ctx) => {
      ...
      // clear_findings request and can only be driven by slash commands.
        ...
        if (id) runtime.findings.delete(id); else runtime.findings.clear();
        pi.appendEntry(FINDINGS_CLEAR_TYPE, { id }); broadcastFindings(runtime); ctx.ui.notify("Pi findings cleared", "info");
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      for (const finding of published) if (!expectedTextMatches(runtime.root, finding)) throw new Error(`finding expected_text does not match ${finding.path}:${finding.start_line}-${finding.end_line}`);
